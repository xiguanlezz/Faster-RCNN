...
class FasterRCNN(nn.Module):
    def __init__(self, path):
        ...
        self.fast_rcnn = FastRCNN(n_class=9, roi_size=7, spatial_scale=1. / 16, classifier=classifier)
    ...
    def forward(self, x, gt_boxes, labels):
        ...
        gt_anchor_locs, gt_anchor_labels = self.anchor_target_creator(gt_boxes[0],
                                                                      anchors,
                                                                      img_size)
        ...
        # ROI LOSS
        gt_roi_labels = torch.from_numpy(gt_roi_labels).long().cuda()
        gt_roi_locs = torch.from_numpy(gt_roi_locs).float().cuda()
        roi_clc_loss = F.cross_entropy(roi_scores, gt_roi_labels)
        ...
        losses = [rpn_loc_loss, rpn_cls_loss, roi_loc_loss, roi_clc_loss]
